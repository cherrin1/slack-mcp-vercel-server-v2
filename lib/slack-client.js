export class SlackClient {
  ...
  // Helper method to get available API methods based on token type
  getSupportedMethods() {
    if (this.isUserToken) {
      return {
        search: true,
        ...
        users: true,
        auth: true,
        // User tokens can access most read operations
        limitations: [
          'Cannot use some admin methods',
          'Cannot post as a bot',
          'Limited to your personal permissions'
        ]
      };
    }
    ...
    if (this.isBotToken) {
      return {
        search: false, // Many bot tokens can't use search
        conversations: true,
        users: true,
        auth: true,
        chat: true,
        limitations: [
          'May not have search permissions',
          'Limited to bot\'s installed permissions'
        ]
      };
    }

    return { limitations: ['Unknown token type'] };
  }
}
